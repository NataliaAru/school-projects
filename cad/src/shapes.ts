...
export class Factory {
  private STANDARD_constant = 30;
  construct() {
  }
  createTriangle(x: number, y: number): DrawableShape {
    let shape = new Triangle(x, y, x - this.STANDARD_constant, y - this.STANDARD_constant, x + this.STANDARD_constant, y - this.STANDARD_constant);
    return shape;
  }
  createCircle(x: number, y: number): DrawableShape {
    let shape = new Circle(x, y, this.STANDARD_constant);
    return shape;
  }
  createRectangle(x: number, y: number): DrawableShape {
    let shape = new Rectangle(x, y, this.STANDARD_constant * 2, this.STANDARD_constant);
    return shape;
  }
}